function __print_elite_functions_help() {
cat <<EOF
Additional EliteCAF functions:
...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/elite/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function mka() {
    if [ -f $ANDROID_BUILD_TOP/$QTI_BUILDTOOLS_DIR/build/update-vendor-hal-makefiles.sh ]; then
        ...
        vendor_hal_script=$ANDROID_BUILD_TOP/vendor/elite/scripts/vendor_hal_makefile_generator.sh
        regen_needed=1
    fi
